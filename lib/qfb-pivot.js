...
let PivotFilter = {
  ...
  /**
   * By default just pivot on the sender...
   */
  getDefaults: function PivotFilter_getDefaults() {
    return {
      latched: null,
      states : {
        senders: 1, recipients: 0, time: 0,
      }
    };
  },
  ...
  onCommand: function PivotFilter_onCommand(aState, aNode, aEvent, aDocument) {
    ...
    if (checked) {
      ...
      if (msgHdrs.length) {
        let [senders, recipients] =
          ls.MsgHdrSlicerDicer.getPeopleFromMessages(msgHdrs, true);
        aState.latched = {
          senders: senders,
    ...
    if (!aState.latched && checked)
      aNode.checked = checked = false;
  ...
  domBindExtra: function PivotFilter_domBindExtra(aDocument, aMuxer, aNode) {
    ...
    function emailPivotHandler() {
      let fv = aMuxer.getFilterValueForMutation(PivotFilter.name);
      if (!fv) {
        fv = PivotFilter.getDefaults();
        aMuxer.setFilterValue(PivotFilter.name, fv);
      }

      let emailAddr = window.findEmailNodeFromPopupNode(aDocument.popupNode,
                                                        "emailAddressPopup")
                            .getAttribute("emailAddress");
      let msg = window.gMessageDisplay.displayedMessage;
      fv.latched = {
        senders: [emailAddr],
        recipients: [emailAddr],
        date: [msg.date, msg.date]
      };
      aMuxer._showFilterBar(true);
      aMuxer.updateSearch();
    }
    aDocument.getElementById("qfb-pivot-email-pivot").addEventListener(
      "command", emailPivotHandler, false);
  },
